<template>
  ...
  <el-form-item label="编号：">
    <el-input v-model="formInline.number" placeholder="请输入编号" size="mini"></el-input>
  </el-form-item>
  <el-form-item>
...
<div style="margin-top:-10px;margin-left:10px"> 
    ...
    <el-button
        ...
        >
          <el-table-column
            ...
            align="center"
          />
          <el-table-column prop="number" label="编号"  align="center" :show-overflow-tooltip="true">
          </el-table-column>
  ...
  <el-form-item label="编号" prop="number">
    <el-input v-model="formDate.number" placeholder="请输入编号" size="mini"></el-input>
  </el-form-item>
  <el-form-item label="国家" prop="country">
    <el-input v-model="formDate.country" placeholder="请输入国家" size="mini"></el-input>
  </el-form-item>
  <el-form-item label="模板图片"  v-if="dialogTitil =='新增强敌'">
     <el-upload
        class="upload-demo"
        :file-list="fileList"
        :on-change="changFile"
  ...
  <el-form-item label="描述" prop="description">
    <el-input v-model="formDate.description" placeholder="请输入描述" size="mini"></el-input>
  </el-form-item>
...
export default {
  ...
  methods:{
      create(){
          this.dialogTitil = '新增强敌'
          this.dialog = true
      },
      handleEditFile(e,i){
          console.log(e,i);
          this.editData = i
          this.dialogTitil = '修改强敌模板图片'
          this.dialogfile = true
      },
      addUser(){
          console.log(this.formDate.country);
            this.$refs.form.validate((valid) => {
        if (valid) {
            this.formDate['type'] = '2'
            let url = this.dialogTitil == '新增强敌'?'/weapon/addWeapon':'/weapon/updateWeapon'
            let type = this.dialogTitil == '新增强敌'?'multipart/form-data':'application/json'
             let formData
             if(this.dialogTitil == '新增强敌'){
                 formData = new FormData();
                 if(this.fileData.length){
                       formData.append('image',this.fileData[0].raw)
                 }
                formData.append('type',this.formDate.type)
             formData.append('country',this.formDate.country)
      ...
      getList(){
          this.$http.post(baseUrl+ '/weapon/findWeapon',{pageInfo:this.pageInfo,type:'2',number:this.formInline.number},{
        headers: {
          "Token": window.sessionStorage.getItem("token")
        }}).then((res)=>{
            console.log(res);
